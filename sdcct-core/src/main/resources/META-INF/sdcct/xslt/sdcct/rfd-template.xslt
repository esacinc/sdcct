<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:x="http://healthIT.gov/sdc">
	<xsl:output method="html"/>
	<xsl:template match="//x:FormDesign">
		<html>
			<head>
	...
	<xsl:template match="x:ChildItems" name="ChildItemsType">
	...
	<xsl:template match="x:Question" name="QuestionItemType">
	...
	<xsl:template name="QuestionItemBaseType">
		...
		<xsl:choose>
			<xsl:when test="x:ListField">
				<xsl:apply-templates select="x:ListField">
					<xsl:with-param name="parentId" select="@ID"/>
					<xsl:with-param name="title" select="@title"/>
				</xsl:apply-templates>
			</xsl:when>
			<xsl:otherwise>
				<xsl:apply-templates select="x:ResponseField">
					<xsl:with-param name="parentId" select="@ID"/>
				</xsl:apply-templates>
			</xsl:otherwise>
		</xsl:choose>
	...
	<xsl:template match="x:ListField">
	...
	<xsl:template match="x:ListItemResponseField">
	...
	<xsl:template match="x:Response">
		<xsl:param name="parentId"/>
		<xsl:variable name="name" select="x:string/@name"/>
	...
	<xsl:template match="x:TextAfterResponse">
		<xsl:value-of select="@val"/>
	</xsl:template>

	<xsl:template match="x:ResponseUnits">
	</xsl:template>

	<xsl:template match="x:ResponseField">
		<xsl:param name="parentId"/>
		<xsl:call-template name="ResponseFieldType"/>
		<xsl:apply-templates> <!-- Response, Extension, TextAfterResponse, ResponseUnits..... -->
			<xsl:with-param name="parentId" select="$parentId"/>
	...
	<xsl:template match="x:OtherText">
